...
export default function AdminPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<DeptLead | null>(null);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    const res = await fetch('/api/department-leads', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: deleteTarget.id }) });
    if (res.ok) { showToast(`${deleteTarget.name} deleted`); setDeleteTarget(null); fetch('/api/department-leads').then(r => r.json()).then(setLeads); }
    else { const data = await res.json(); showToast(data.error || 'Failed to delete'); }
    setDeleting(false);
  };
  ...
  return (
    <div className="fade-in">
      ...
      <div className="table-container">
        <table>
          <thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Role</th><th>Department</th><th style={{textAlign:'right'}}>Members</th><th>Joined</th><th style={{textAlign:'center'}}>Actions</th></tr></thead>
          <tbody>
            {leads.map(s => (
              <tr key={s.id}>
                ...
                <td style={{ textAlign: 'center' }}>
                  {s.id !== (session?.user as any)?.id && (
                    <button onClick={() => setDeleteTarget(s)} className="btn btn-sm" style={{ background: 'transparent', color: 'var(--text-tertiary)', padding: '4px 8px', minWidth: 'auto' }} title="Delete">
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setDeleteTarget(null)}>
          <div className="modal" style={{ maxWidth: '400px' }}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setDeleteTarget(null)}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
            </div>
            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', margin: '0 0 20px' }}>Are you sure you want to delete <strong>{deleteTarget.name}</strong>? This action cannot be undone.</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button type="button" onClick={() => setDeleteTarget(null)} className="btn btn-outline" style={{ flex: 1 }}>Cancel</button>
              <button type="button" onClick={handleDelete} className="btn" style={{ flex: 1, background: '#ef4444', color: '#fff', border: 'none' }} disabled={deleting}>{deleting ? 'Deleting…' : 'Delete'}</button>
            </div>
          </div>
        </div>
      )}
